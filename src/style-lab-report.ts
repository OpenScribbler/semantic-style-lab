...
export function buildEditorChecklist(projects: ProjectAudit[]) {
	return projects.flatMap((project) => {
		const actionable = project.findings.filter((finding) => finding.action !== 'suppress');
...
export function buildHtmlReport(report: {
	...
	summary: { files: number; candidates: number; flag: number; review: number; suppress: number; input_tokens: number; estimated_input_cost_usd: number };
}) {
