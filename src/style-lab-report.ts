...
<div class="eyebrow">Semantic Style Lab · ${escapeHtml(report.pipeline_version)} · ${escapeHtml(report.rule_set_version)} · audit</div><h1>Findings grouped by page, with Jev's probability where it ran.</h1>
...
<section class="panel"><h2>Rule set</h2><ul>${ruleItems}</ul><p class="muted">Suppression is a recommendation only. This command does not edit source files or change CI.</p></section>
