...
function lexicalRanges(source: string, ranges: ProtectedRange[], format: MarkdownFormat, protectMarkdownCodeLexically: boolean) {
	...
	// Hugo mermaid and highlight shortcode bodies are diagram or code source, not prose.
	regexRanges(source, /{{<\s*(mermaid|highlight)\b[^}]*>}}[\s\S]*?{{<\s*\/\1\s*>}}/g, 'code_or_pre_block', ranges);
